...
export default class ReservationReportsController {
  ...
  async generateArrivalList({ request, response, auth }: HttpContext) {
    try {
      ...
      const reportData = await this.buildArrivalListData(payload, payload.hotelId)
   ...
   */
   ...
   async generateDepartureList({ request, response, auth }: HttpContext) {
    try {
      const payload = await request.validateUsing(createReservationReportValidator)
      const reportData = await this.buildDepartureListData(payload, payload.hotelId)
   ...
   */
   ...
   async generateCancelledList({ request, response, auth }: HttpContext) {
    try {
      const payload = await request.validateUsing(createReservationReportValidator)
      const reportData = await this.buildCancellationListData(payload, payload.hotelId)

      return response.ok({
        success: true,
        message: 'Cancellation list report generated successfully',
        data: reportData,
        generatedAt: DateTime.now().toISO(),
        generatedBy: auth.user?.firstName + ' ' + auth.user?.lastName
      })
   ...
   */
   async generateVoidList({ request, response, auth }: HttpContext) {
    try {
      const payload = await request.validateUsing(createReservationReportValidator)
      const reportData = await this.buildVoidListData(payload, payload.hotelId)

      return response.ok({
        success: true,
        message: 'Void list report generated successfully',
        data: reportData,
        generatedAt: DateTime.now().toISO(),
        generatedBy: auth.user?.firstName + ' ' + auth.user?.lastName
      })
      ...
      return response.badRequest({
        success: false,
        message: 'Failed to generate voided reservations report',
        error: error.message
      })
    }
  }
  ...
  async generateArrivalListPdf({ request, response, auth }: HttpContext) {
    try {
      ...
      if (!reportData) {
        return response.badRequest({ success: false, message: 'Failed to generate report data' })
      }
      ...
      const pdfBuffer = await PdfGenerationService.generatePdfFromHtml(htmlContent, {
        format: 'A4',
        landscape: true,
        margin: { top: '60px', right: '10px', bottom: '70px', left: '10px' },
  ...
  // Méthode privée qui retourne juste les données
  private async buildArrivalListData(payload: any, hotelId: number) {
    const startDateTime = DateTime.fromISO(payload.startDate)
    const endDateTime = DateTime.fromISO(payload.endDate)
    ...
    let query = Reservation.query()
      ...
      .preload('reservationRooms', (reservationRoomQuery) => {
        reservationRoomQuery.preload('room', (roomQuery) => { roomQuery.preload('roomType') })
        reservationRoomQuery.preload('roomType')
        reservationRoomQuery.preload('roomRates', (roomRateQuery) => { roomRateQuery.preload('rateType') })
      })
      ...
      .whereDoesntHave('reservationRooms', (roomQuery) => {
        roomQuery.whereHas('roomType', (rt) => rt.where('is_paymaster', true))
      })


    if (payload.roomTypeId) {
      query = query.whereHas('reservationRooms', (roomQuery) => {
        roomQuery.where('roomTypeId', payload.roomTypeId)
      })
    }
    if (payload.ratePlanId) query = query.whereHas('reservationRooms', (q) => {
      q.whereHas('roomRates', (rq) => { rq.where('rateTypeId', payload.ratePlanId) })
    })
    if (payload.company) query = query.where('companyCode', payload.company)
    ...
    const reservationList = reservations.map((reservation) => {
      ...
      if (payload.selectedColumns?.length > 0) {
        payload.selectedColumns.forEach((col: string) => {
          switch (col) {
            case 'pickUp': baseData.pickUp = reservation.pickupTime || ''; break
            case 'dropOff': baseData.dropOff = reservation.dropoffTime || ''; break
            case 'resType': baseData.reservationType = reservation.reservationType || ''; break
          }
        })
      }
   ...
   private async buildDepartureListData(payload: any, hotelId: number) {
    const startDateTime = DateTime.fromISO(payload.startDate)
    const endDateTime = DateTime.fromISO(payload.endDate)
   ...
   private async buildCancellationListData(payload: any, hotelId: number) {
    const startDateTime = DateTime.fromISO(payload.startDate)
    const endDateTime = DateTime.fromISO(payload.endDate)
    ...
    let query = Reservation.query()
        ...
        .whereBetween('cancellationDate', [startDateTime.toSQL()!, endDateTime.toSQL()!])
    ...
    const reservations = await query.orderBy('createdAt', 'asc')
  ...
  private async buildVoidListData(payload: any, hotelId: number) {
    const startDateTime = DateTime.fromISO(payload.startDate)
    const endDateTime = DateTime.fromISO(payload.endDate)
    ...
    let query = Reservation.query()
        ...
        .where('hotelId', hotelId)
        .where('status', 'voided')
        .whereBetween('created_at', [startDateTime.toSQL()!, endDateTime.toSQL()!])



    const reservations = await query.orderBy('created_at', 'asc')
     ...
     const reservationList = reservations.map((reservation) => {
        ...
        return {
          ...
          paid: totalPayments.toFixed(2),
          balance: (totalCharges - totalPayments).toFixed(2),
          source: reservation.bookingSource?.sourceName || '',
