  ...
  public async cancelReservation(ctx: HttpContext) {
    ...
    try {
      ...
      for (const resService of reservation.reservationRooms) {
        if (selectedRooms.includes(resService.id) && resService.status !== ReservationProductStatus.CANCELLED) {
          ...
          await LoggerService.log({
            ...
            hotelId: reservation.hotelId,
            description: `ReservationRoom #${resService.id} cancelled.`,
            ctx: ctx,
          })
        }
      }
...
public async getReservationById({ request, response, auth, params }: HttpContext) {
          ...
          if (roomIds.length > 0) {
            ...
            if (arrivedDate.toISODate() === departDate.toISODate()) {
              ...
              existingReservationsQuery = existingReservationsQuery
                ...
                .where((query) => {
                  query.whereBetween('checkInTime', [
              ...
              existingReservationsQuery = existingReservationsQuery.where((query) => {
                query
                  .whereBetween('checkInDate', [
                    arrivedDate.toISODate(),
                    departDate.toISODate(),
                  ])
                  .orWhereBetween('checkOutDate', [
                    arrivedDate.toISODate(),
                    departDate.toISODate(),
                  ])
                  .orWhere((overlapQuery) => {
    ...
    try {
      ...
      if (exchangeType === 'reservation_swap') {
        ...
        if (!targetReservationId) {
          await trx.rollback()
          return response.badRequest({
            message: 'Target reservation ID is required for reservation swap',
          })
        }
        ...
        if (!targetReservation) {
          await trx.rollback()
          return response.badRequest({
            message: 'Target reservation not found or not in same hotel',
          })
        }
        ...
        if (!targetReservationRoom) {
          await trx.rollback()
          return response.badRequest({
            message: 'No active room assignment found for target reservation',
          })
        }
        ...
        if (!newRoomId) {
          await trx.rollback()
          return response.badRequest({
            message: 'New room ID is required for room upgrade/downgrade',
          })
        }
        ...
        if (!newRoom) {
          await trx.rollback()
          return response.badRequest({
            message: 'New room not found or not available in this hotel',
          })
        }
        ...
        const conflictingReservation = await ReservationRoom.query({ client: trx })
          ...
          .where((query) => {
            query
              .where((subQuery) => {
                subQuery
                  .where('check_in_date', '<=', reservation.arrivedDate?.toISODate()!)
                  .where('check_out_date', '>', reservation.arrivedDate?.toISODate()!)
              })
              .orWhere((subQuery) => {
                subQuery
                  .where('check_in_date', '<', reservation.departDate?.toISODate()!)
  ...
  //       await ReservationRoom.query({ client: trx }).where('reservationId', reservationId).update({
  //         status: 'no_show',
  //         lastModifiedBy: auth.user?.id,
